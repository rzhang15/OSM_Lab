...
def plot_routine(n_agents, grid, dim, num_points):
    for itheta in range(ntheta):
        f=open("forplot"+str(theta_range[itheta])+".txt", 'w')
        ...
        vals=grid[itheta].evaluateBatch(k_points)
...
def ls_error(n_agents, t1, t2, num_points):
    for itheta in range(ntheta)
        ...
        for i in range(t1, t2):
            ...
            v_prev.read("valnew_" + str(theta_range[itheta]) + '_'+str(i)+".txt")
            v_next.read("valnew_" + str(theta_range[itheta]) + '_'+str(i+1)+".txt")
